...
server.listen(6000, () => console.log(`Running on port: ${PORT}`));
...
function score_calculate(lat, long, lat_ans, long_ans, time) {
  ...
  let score = 0;
  let dist = Number(
    Math.ceil(Math.sqrt((lat - lat_ans) ** 2 + (long - long_ans) ** 2))
  );
  console.log("dist is ", dist);
  if (dist < 2) score = 50 + time;
  else if (dist >= 2 && dist < 3) score = 40 + time;
  else if (dist >= 3 && dist <= 4) score = 30 + time;
  ...
  .then(() => {
    ...
    io.on("connection", function (socket) {
      ...
      if (
        ...
        io.nsps["/"].adapter.rooms["room-" + roomno].length > 5
      )
        roomno = uuidv4();
          ...
          try {
            ...
            if (rooms["room-" + roomno].timer === 0) {
              if (rooms["room-" + roomno].round > 0) {
                ...
                io.sockets
                  .in("room-" + roomno)
                  .emit("markers", rooms["room-" + roomno].markers);
              ...
              var randIndex = parseInt(
                Number(Math.random() * (dbdata.length - 1)),
                10
              );
      ...
      socket.on("setUsername", function (data) {
        ...
        io.sockets
          .to("room-" + roomno)
          .emit("userSet", { username: data.username });
        rooms["room-" + roomno][data.username] = Math.random();
        rooms["room-" + roomno].scores.push({
          name: data.username,
          score: 0,
          avaterID: rooms["room-" + roomno][data.username],
        });

        io.sockets
          .in("room-" + roomno)
          .emit("scores", rooms["room-" + roomno].scores);
        console.log(rooms);
      });
      ...
      socket.on("mapclicked", (data) => {
        try {
          ...
          let curr_score = score_calculate(
          ...
          score_arr.forEach((dat, index) => {
            ...
            if (dat.name === socket.playerName) {
              ...
              if (
                curr_score != 0 &&
                !rooms["room-" + roomno].update_done.includes(socket.playerName)
              ) {
      ...
      socket.on("disconnect", (data) => {
        ...
        console.log(socket.playerName);
        try {
          const usersInRoom = rooms["room-" + socket.roomKey].scores.filter(
            (item) => item.name !== socket.playerName
          );
